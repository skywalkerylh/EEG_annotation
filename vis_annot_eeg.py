...
# app func
def create_annotation_dataframe(start_pos, end_pos):
    start_time = time_ch1[start_pos]
    end_time = time_ch1[end_pos]
    duration_in_ms = int((end_pos - start_pos)/sf * 1000)
...
def display_annotation_content(start_pos, start_time, end_pos, end_time, duration_in_ms):
...
def record_delete_annotation(annotation, annot_export):
    # annot_export is empty then add annot directly
    if annot_export.shape[0]==0 :

        annot_export= annot_export.append(annotation,ignore_index=True)
        ...
        is_in_pd= annot_export.isin(annotation.to_dict(orient='list')).all(axis=1)
        print(is_in_pd)
        if is_in_pd.any():
...
def update_graph(n_clicks,width, pixelsperinch):
    ...
    fig.add_trace(go.Scatter(x=x_values_row, y=y_values_row_ch1, mode='lines'), row=1, col=1)
    fig.add_trace(go.Scatter(x=x_values_row, y=y_values_row_ch2, mode='lines'), row=2, col=1)
...
@app.callback(
    dash.dependencies.Output('annotation-info', 'children'),
    [dash.dependencies.Input('plot-container', 'relayoutData')],
    [dash.dependencies.State('annotation-info', 'children')])

def rect_annotation_added(fig_data, content):
    global annot_export
    if fig_data is None:
    ...
    if 'shapes' in fig_data :
        ...
        annotation= create_annotation_dataframe(start_pos,end_pos)
        
        content= ''
        content= display_annotation_content(
            start_pos, annotation['start_time'].item(),
            end_pos, annotation['end_time'].item(),
            annotation['duration_ms'].item()
        )
        annot_export= record_delete_annotation(annotation, annot_export)
